...
def get_zhongxinqingdao_chuzhigonggao(queue_id, webpage_id):
    page = ChromiumPage(co)
    # page.set.headers(headers=headers)
    page.set.load_mode.none()
    try:
        for count in range(1, 5 + 1):
            page_url = f'http://www.qdamc.citic/announcement-54-1-{count}.html'
            ...
            for title in title_list:
                ...
                if title_url not in title_set:
                    ...
                    annex = res_title_html.xpath("//div[@class='main2']/div[@class='newshow']//a/@href")
                    if annex:
                        files = []
                        original_url = []
                        for ann in annex:
...
# get_zhongxinqingdao_chuzhigonggao(111, 222)
