...
def get_zhejiangzheshang_zichantuijie(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    count = 1
    while True:
        page_url = f'https://www.zsamc.com/infor/index/21/0/{count}.html'
        ...
        for title in title_list:
            ...
            if title_url not in title_set:
                title_date = "".join(
                    title.xpath("./div[@class='shoot-ls-rg']/div[@class='shoot-ls-btn']/text()")).strip()
                # 使用re匹配日期
                ...
                if annex:
                    ...
                    for ann in annex:
                        ...
                        if file_type in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'rar', '7z',
                                         'png', 'jpg', 'jpeg']:
                            file_url = upload_file_by_url(ann, "zhejiang_tuijie", file_type)
                            # file_url = 111
                            files.append(file_url)
                            original_url.append(ann)
                else:
...
# get_zhejiangzheshang_zichantuijie(111, 1111)
