...
headers = {
    ...
    'pragma': 'no-cache',
    'priority': 'u=0, i',
    'referer': 'https://www.ywcq.com/Article/Search/Index/2?keyword=%E5%80%BA%E6%9D%83',
...
def get_yiwuchanquanjiaoyisuo(queue_id, webpage_id):
    ...
    try:
        ...
        for page_num in range(1, 3 + 1):
            if page_num == 1:
                url = 'https://www.ywcq.com/Article/Search?keyword=%E5%80%BA%E6%9D%83'
            else:
                url = f'https://www.ywcq.com/Article/Search/Index/{page_num}?keyword=%E5%80%BA%E6%9D%83'
            img_set = set()
            name = '义乌产权交易所'
            ...
            data_list = res_html.xpath("//div[@id='articles']/ul[@class='listUl']/li")
            ...
            for data in data_list:
                ...
                if 'http' not in page_url:
                    page_url = 'https://www.ywcq.com' + page_url
                else:
                    continue
                ...
                title_date = ''.join(data.xpath("./span/text()"))
                # 使用re模块提取日期
                ...
                if title_url not in title_set:
                    title_content = "".join(
                        res_html.xpath("//div[@class='contentBox']//text()"))

                    annex = res_html.xpath("//div[@class='contentBox']//@href | //div[@class='contentBox']//@src")
                    if annex:
                        ...
                        for ann in annex:
                            if not ann:
                                continue
                            if "http" not in ann:
                                ann = 'https://www.ywcq.com/' + ann
                            file_type = ann.split('.')[-1]
                            file_type = file_type.strip()
                            if file_type in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'rar', '7z',
                                             'png', 'jpg', 'jpeg']:
                                file_url = upload_file_by_url(ann, "yiwu", file_type)
                                # file_url = 111
                    ...
                    content_1 = res_html.xpath("//div[@class='contentBox']")
                    content_html = ""
                    # print(content_html)
                    ...
                    try:
                        ...
                        image = get_image(page, title_url,
                                          "xpath=//div[@class='contentBox']", is_to_bottom=True)
                    except:
                        print('截取当前显示区域')
