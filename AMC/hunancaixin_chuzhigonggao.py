...
def get_hunancaixin_chuzhigonggao(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    try:
        for count in range(1, 12 + 1):
            page_url = f'https://amc.hnchasing.com/cxamc/zcxx53/zcczgg79/fd05d517-{count}.html'
            response = requests.get(page_url, headers=headers)
            res = response.content.decode()
            res_html = etree.HTML(res)
            title_list = res_html.xpath("//ul[@class='pub_news']/li/a")
            img_set = set()
            name = '湖南省财信资产管理有限公司'
            ...
            for title in title_list:
                title_name = "".join(
                    title.xpath("./p/text()"))
                title_date = "".join(title.xpath("./time/text()"))
                # 使用re模块提取日期
                ...
                if title_url not in title_set:
                    ...
                    annex = res_title_html.xpath("//div[@class='top']/div[@class='bottom']//a/@href")
                    if annex:
                        files = []
                        original_url = []
                        for ann in annex:
...
# get_hunancaixin_chuzhigonggao(111, 222)
