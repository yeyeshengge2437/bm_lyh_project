...
def get_changshaxiangjiang_zichantuijie(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    try:
        for count in range(1, 1 + 1):
            page_url = f'http://www.xiangjiang-amc.com/assetlist/31614'
            response = requests.get(page_url, headers=headers)
            res = response.content.decode()
            res_html = etree.HTML(res)
            title_list = res_html.xpath("//div[@class='zclb_row']/table/tbody/tr")
            img_set = set()
            name = '长沙湘江资产管理有限公司_资产推介'
            title_set = judge_title_repeat(name)
            for title in title_list:
                title_name = "".join(
                    title.xpath("./td[1]/a//text()"))
                title_date = "".join(title.xpath("./td[6]//text()"))
                # 使用re模块提取日期
                ...
                if title_date:
                    ...
                    title_date = ''
                title_url = "".join(title.xpath("./td[1]/a/@href")).strip()
                if title_url not in title_set:
                    ...
                    title_content = "".join(res_title_html.xpath(
                        "//div[@class='main1200 bg_fff']//text()"))

                    annex = res_title_html.xpath(
                        "//div[@class='main1200 bg_fff']//a/@href | //div[@class='main1200 bg_fff']//@src")
                    if annex:
                        ...
                        for ann in annex:
                            ...
                            if file_type in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'rar', '7z',
                                             'png', 'jpg'] and 'xjjt' in ann:
                                file_url = upload_file_by_url(ann, "changsha_zichan", file_type)
                                # file_url = 111
                    ...
                    content_1 = res_title_html.xpath("//div[@class='main1200 bg_fff']")
                    content_html = ''
                    ...
                    for con in content_1:
                        content_html += etree.tostring(con, encoding='utf-8').decode()
                    try:
                        image = get_image(page, title_url, "xpath=//div[@class='main1200 bg_fff']", left_offset=10)
                    except:
...
# get_changshaxiangjiang_zichantuijie(111, 222)
