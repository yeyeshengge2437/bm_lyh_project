...
def get_suzhouchanquanjiaoyizhongxin(queue_id, webpage_id):
    ...
    try:
        ...
        for zq_type in ['C06']:
            ...
            res = requests.get('https://www.szee.com.cn/jrzc/', headers=headers)
            # print(res.text)
            res_json = res.content.decode()
            # print(res_json)
            res_html = etree.HTML(res_json)
            data_list = res_html.xpath("//div[@id='main-zscq']//ul/li")
            ...
            for data in data_list:
                ...
                if not page_str:
                    continue
                page_url = 'https://www.szee.com.cn' + page_str

                title_name = ''.join(data.xpath(".//div[@class='zscq-rt']/h3/text()"))
                # import datetime; print(datetime.datetime.utcfromtimestamp(1740326400000 // 1000).strftime('%Y-%m-%d'))
                title_date = ''.join(data.xpath(".//div[@class='zscq-lf']//text()"))
                title_date = str(datetime.strptime(title_date, "%d%Y/%m").strftime("%Y-%m-%d"))
                # # 使用re模块提取日期
                ...
                time.sleep(3)
                res_html = etree.HTML(res.content.decode())

                title_url = page_url
                if title_url not in title_set:
                    title_content = "".join(res_html.xpath("//div[@id='main-news']//div[@class='news-content']//text()"))

                    annex = res_html.xpath("//div[@id='main-news']//div[@class='news-content']//@href | //div[@id='main-news']//div[@class='news-content']//@src")
                    if annex:
                    ...
                    content_1 = res_html.xpath("//div[@id='main-news']//div[@class='news-content']")
                    content_html = ""
                    # print(content_html)
                    ...
                    try:
                        ...
                        image = get_image(page, title_url,
                                          "xpath=//div[@id='main-news']//div[@class='news-content']")
                    except:
                        print('截取当前显示区域')
...
# get_suzhouchanquanjiaoyizhongxin(111, 222)
