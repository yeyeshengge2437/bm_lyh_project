...
headers = {
    ...
    'origin': 'https://sta.hnprec.com',
    'platformid': 'AssetsCliectApp',
    'pragma': 'no-cache',
    'priority': 'u=1, i',
    'referer': 'https://sta.hnprec.com/',
    ...
    'webbusinesstype': 'Assets',
}



def get_henanshengchanquanjiaoyizhongxin(queue_id, webpage_id):
    ...
    try:
        ...
        for zq_type in ['C06']:
            ...
            res = requests.post('https://biz.hnprec.com/buyerapi/api/IndexApi/QueryAnncPage', headers=headers, json=json_data)
            # print(res.text)
            res_json = res.json()
            # print(res_json)
            ...
            for data in data_list:
                ...
                if title_url not in title_set:
                    title_content = "".join(res_html.xpath("//div[@id='appContent']//text()"))

                    annex = res_html.xpath("//div[@id='appContent']//a//@href")
                    annex_name = ''.join(res_html.xpath("//div[@id='appContent']//a/span/text()"))
                    if annex:
                        ...
                        for ann in annex:
                            ...
                            file_type = annex_name.split('.')[-1]
                            file_type = file_type.strip()
                            if file_type in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'rar', '7z',
                                             'png', 'jpg', 'jpeg'] and 'Download' in ann:
                                file_url = upload_file_by_url(ann, "henanchanjiao", file_type)
                                # file_url = 111
                    ...
                    for con in content_1:
                        content_html += etree.tostring(con, encoding='utf-8').decode()
                    try:
                        image = get_image(page, title_url, "xpath=//div[@id='appContent']")
                    except:
                        print('截取当前显示区域')
...
# get_henanshengchanquanjiaoyizhongxin(111, 222)
