...
def get_xinyezichan_zichantuijie(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    try:
        for count in range(917596318, 917596313 - 1, -1):
            page_url = f'http://www.ciamc.com.cn/ciamc/insetInfo/Refelral-Info.html_{count}.html'
            response = requests.get(page_url, headers=headers)
            res = response.content.decode()
            res_html = etree.HTML(res)
            title_list = res_html.xpath("//div[@class='list']/div[@class='list2']/ul/li")
            img_set = set()
            name = '兴业资产管理有限公司_资产推介'
            title_set = judge_title_repeat(name)
            for title in title_list:
                title_name = "".join(
                    title.xpath("./a/text()")).strip()
                title_date = "".join(title.xpath("./span[@class='time']/text()"))
                # 使用re模块提取日期
                ...
                if title_date:
                    ...
                    title_date = ''
                title_url = "http://www.ciamc.com.cn" + "".join(title.xpath("./a/@href"))
                if title_url not in title_set:
                    ...
                    if annex:
                        ...
                        for ann in annex:
                            ...
                            if file_type in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'rar', '7z',
                                ...
                                file_url = upload_file_by_url(ann, "xingye_zichan", file_type)
                                # file_url = 111
                    ...
                    # print(title_name, title_url, title_date, files, original_url)
                    # return
                    # 上传到测试数据库
...
# get_xinyezichan_zichantuijie(111, 222)
