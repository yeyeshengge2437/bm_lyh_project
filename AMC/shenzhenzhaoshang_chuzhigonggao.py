...
headers = {
    ...
    # 'Cookie': '__jsluid_s=457413376e76a3ea7fadf7fe35e342a4',
    'Pragma': 'no-cache',
...
def get_shenzhenzhaoshang_chuzhigonggao(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    try:
        for count in range(1, 1 + 1):
            page_url = f'https://www.cmamc.net.cn/zichan_ye.php?fid=8'
            response = requests.get(page_url, headers=headers)
            res = response.content.decode()
            res_html = etree.HTML(res)
            title_list = res_html.xpath("//ul[@class='zichan2']/li[position() >= 2]/a")
            img_set = set()
            name = '深圳市招商平安资产管理有限责任公司'
            title_set = judge_title_repeat(name)
            for title in title_list:
                title_name = "".join(
                    title.xpath("./div[@class='zichan_name']/text()"))
                title_date = "".join(title.xpath("./div[@class='zichan_date']/text()"))
                # 使用re模块提取日期
                ...
                if title_date:
                    ...
                    title_date = ''
                title_url = "https://www.cmamc.net.cn/" + "".join(title.xpath("./@href"))
                if title_url not in title_set:
                    ...
                    title_html_info = res_title_html.xpath(
                        "//div[@class='ny_detail']/div[@class='t']")
                    content_1 = res_title_html.xpath("//div[@class='ny_detail']/div[@class='con']")
                    content_html = ''
...
# get_shenzhenzhaoshang_chuzhigonggao(111, 222)
