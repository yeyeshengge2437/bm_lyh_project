...
def get_jiangsuzichan_zichantuijie(queue_id, webpage_id):
    ...
    try:
        for count in range(1, 36 + 1):
            ...
            response = requests.post('https://www.jsamc.com.cn/prod-api/assets_promote/list', headers=headers,
                                     json=json_data)
            res_json = response.json()
            title_list = res_json["body"]["list"]
            img_set = set()
            name = "江苏资产管理有限公司_资产推介"
            title_set = judge_title_repeat(name)
            for title in title_list:
                title_name = title["name"]
                title_date = title["showTime"]
                title_url = f"https://www.jsamc.com.cn/assets-promote/promote-information/{title['id']}"
                if title_url not in title_set:
                    ...
                    title_content = ''.join(
                        res_title_html.xpath("//div[@class='assetsPost_contentContainer__EoCnw']//text()"))
                    ...
                    if annex:
                        ...
                        for ann in annex:
                            if "http" not in ann:
                                ann = 'https://www.jsamc.com.cn/' + ann.strip('../..')
                            else:
                                continue
                            file_type = ann.split('.')[-1]
                            if file_type in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'rar', '7z',
                                             'png', ] and 'File' in ann:
                                file_url = upload_file_by_url(ann, "jiangsu_tuijie", file_type)
                                # file_url = 111
...
# get_jiangsuzichan_zichantuijie(111, 222)
