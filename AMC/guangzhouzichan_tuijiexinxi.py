...
def get_guangzhouzichan_zichantuijie(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    try:
        for count in range(1, 4 + 1):
            if count == 1:
                page_url = f'https://www.guangzhouamc.com/asset/tuijiexinxi.html'
            else:
                page_url = f'https://www.guangzhouamc.com/asset/tuijiexinxi-{count}.html'
            response = requests.get(page_url, headers=headers)
            res = response.content.decode()
            res_html = etree.HTML(res)
            title_list = res_html.xpath("//ul[@class='column_list']/li[@class='slide-top']/a")
            img_set = set()
            name = '广州资产管理有限公司_推介信息'
            ...
            for title in title_list:
                ...
                if title_date:
                    ...
                    title_date = ''
                title_date = datetime.strptime(title_date, "%Y-%m-%d")
                ...
                if title_url not in title_set:
                    ...
                    if annex:
                        ...
                        for ann in annex:
                            ...
                            if file_type in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'rar',
                                             '7z', ] and 'upload' in ann:
                                file_url = upload_file_by_url(ann, "guangzhou_tuijie", file_type)
...
# get_guangzhouzichan_zichantuijie(111, 222)
