...
from api_paper import upload_file_by_url, get_image, judge_bm_repeat, upload_file, judge_title_repeat

co = ChromiumOptions()
co = co.set_argument('--no-sandbox')
co = co.headless()
co.set_paths(local_port=9241)


def zhongxinjinrongguangdong_gzh(queue_id, webpage_id):
    img_set = set()
    name = '中国中信金融资产广东分公司'
    ...
    data_list = html.xpath("//a[@class='list_item js_post']")
    for data in data_list:
        title_name = ''.join(data.xpath(".//h2//text()"))
        title_url = ''.join(data.xpath("./@href"))
        page.get(title_url)
        ...
        if annex:
            ...
            original_url = []
            for ann in annex:
                file_url = upload_file_by_url(ann, "guangdong", 'png')
                # file_url = 111
...
# zhongxinjinrongjiangsu_gzh(111, 222)
