...
from api_paper import judge_bm_repeat, upload_file, judge_title_repeat, upload_file_by_url

co = ChromiumOptions()
co = co.set_argument('--no-sandbox')
co = co.headless()
co.set_paths(local_port=9225)
...
def get_zhongyuanzichan_zichantuijie(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    try:
        response = requests.post('https://www.zyamc.net/api/portal/asset/list', headers=headers, json=json_data)
        res_json = response.json()
        title_list = res_json["data"]["content"]
        img_set = set()
        page_url = 'https://www.zyamc.net/#/asset-zone/4'
        name = "中原资产管理有限公司_资产推介"
        title_set = judge_title_repeat(name)
        for title in title_list:
            title_name = title["assetName"]
            title_date = title["createTime"]
            # 使用re模块提取日期
            ...
            if title_date:
                ...
                title_date = ''
            title_url = f"https://www.zyamc.net/#/asset-promotion-detail/{title['assetId']}"
            if title_url not in title_set:
                page.get(title_url)
                time.sleep(4)
                page_html = page.html
                page_html_tree = etree.HTML(page_html)
                content_html_ = page_html_tree.xpath("//div[@class='ap_content']")
                content_html = ''
                for con in content_html_:
                    content_html += etree.tostring(con, encoding='utf-8').decode()
                annex = page_html_tree.xpath("//@href | //@src")
                if annex:
                    ...
                    for ann in annex:
                        if "http" not in ann:
                            ann = 'https://www.zyamc.net' + ann
                        file_type = ann.split('.')[-1]
                        if file_type in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'rar', '7z', 'png',
                                         'jpg', 'jpeg']:
                ...
                title_content = ''.join(page_html_tree.xpath('//div[@class="ap_content"]//text()')).strip()

                image = get_image(page, title_url, "xpath=//div[@class='ap_detail_block']")
                create_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                create_date = datetime.now().strftime('%Y-%m-%d')
                # print(title_name, title_date, title_content, content_html)
                # return
                # 上传到测试数据库
...
# get_zhongyuanzichan_zichantuijie(111, 222)
