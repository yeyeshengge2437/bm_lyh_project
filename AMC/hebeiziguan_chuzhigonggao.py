...
headers = {
    ...
    # 'cookie': 'PHPSESSID=3094mii0hikdkvppmdlbok5ac7',
    'pragma': 'no-cache',
    'priority': 'u=0, i',
...
def get_hebeiziguan_chuzhigonggao(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    try:
        for count in range(1, 5 + 1):
            page_url = f'https://www.hebamc.com/index.php/Cn/Info/index/classid/61/p/{count}.html'
            response = requests.get(page_url, headers=headers)
            res = response.content.decode()
            res_html = etree.HTML(res)
            title_list = res_html.xpath("//div[@class='news_con']/ul[@class='clear-fix']/li")
            img_set = set()
            name = '河北省资产管理有限公司'
            ...
            for title in title_list:
                ...
                if title_date:
                    ...
                    title_date = ''
                title_url = "https://www.hebamc.com" + "".join(title.xpath("./a/@href"))
                if title_url not in title_set:
                    ...
                    title_html_info = res_title_html.xpath(
                        "//h3[@class='nv_tit']")
                    content_1 = res_title_html.xpath("//div[@id='body_con']")
                    content_html = ''
                    ...
                    try:
                        image = get_image(page, title_url,
                                          "xpath=//div[@id='nmain']/div[@class='n_right']", up_offset=40, down_offset=110)
                    except:
                        print('截取当前显示区域')
...
# get_hebeiziguan_chuzhigonggao(111, 222)
