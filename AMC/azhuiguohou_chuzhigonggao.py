...
def get_azhuiguohou_chuzhigonggao(queue_id, webpage_id):
    page = ChromiumPage(co)
    page.set.load_mode.none()
    try:
        for count in range(1, 4 + 1):
            page_url = f'http://www.gohoamc.com/info.php?class_id=102104&page={count}'
            response = requests.get(page_url, headers=headers)
            res = response.content.decode()
            res_html = etree.HTML(res)
            title_list = res_html.xpath("//ul[@class='list']/li")
            img_set = set()
            name = '安徽国厚金融资产管理有限公司'
            ...
            for title in title_list:
                title_name = "".join(
                    title.xpath("./a/text()"))
                title_date = "".join(title.xpath("./span[@class='d']/text()"))
                # 使用re模块提取日期
                ...
                if title_date:
                    ...
                    title_date = ''
                title_url = "http://www.gohoamc.com/" + "".join(title.xpath("./a/@href"))
                if title_url not in title_set and '处置公告' in title_name:
                    ...
                    try:
                        image = get_image(page, title_url,
                                          "xpath=//div[@class='article wow slideInLeft animated']/div[@class='bd']")
                    except:
                        print('截取当前显示区域')
...
# get_azhuiguohou_chuzhigonggao(111, 222)
