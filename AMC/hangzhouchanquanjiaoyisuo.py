    ...
    try:
        ...
        for zq_type in ['C06']:
            params = {
                ...
                'subjectType': '资产转让',
                'subjectDetailType': '债权',
            }
            img_set = set()
            name = '杭州产权交易所'
            title_set = judge_title_repeat(name)

            res = requests.get('https://www.hzaee.com/api/api/subjectmaininfo/page', params=params, headers=headers)
            # print(res.text)
            res_json = res.json()
            print(res_json)
            # print(res_json)
            ...
            for data in data_list:
                ...
                page_url = f'https://www.hzaee.com/assettransfer_detail?id={subject_id}'
                title_name = data["subjectName"]
                # import datetime; print(datetime.datetime.utcfromtimestamp(1740326400000 // 1000).strftime('%Y-%m-%d'))
                title_date = str(data["createDate"])
                # 20250212
