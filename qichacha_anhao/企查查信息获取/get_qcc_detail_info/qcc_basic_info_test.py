...
def qcc_search_company(search_company_name, from_queue, webpage_id):
    ...
    if res_json:
        ...
        for company in all_company_list:
            ...
            if flag:
                ...
                if not pid_list or not tid_list:
                    ...
                    if company_data and susong_data and cfengxian_data and crun_data and cassets_data and chistory_data:
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, changelist_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    up_qcc_res_data(changelist_url, 'business_change', 'post', json_data, key_no,
                                                    webpage_id)
                        ...
                        print(type(susong_wenshulist_num), susong_wenshulist_num)
                        # 少于10个的情况下
                        ...
                        # 股权冻结
                        share_freeze_num = inquire_dict.get('股权冻结')
                        if not share_freeze_num:
                            share_freeze_num = 0
                        if 0 < share_freeze_num <= 10:
                            share_freeze_list = susong_json["datalist"]["assistancelist"]["data"]
                        # 多于10个的情况下
                        elif share_freeze_num > 10:
                            share_freeze_list = []
                            input("股权冻结信息大于十个, 建议处理")
                            share_freeze_page = math.ceil(share_freeze_num / 10)
                            for page_ in range(1, share_freeze_page + 1):
                                share_freeze_url = f"https://www.qcc.com/api/datalist/share_freezelist?isNewAgg=true&keyNo={key_no}&pageIndex={page_}"
                                pass
                        else:
                            share_freeze_list = []
                        dispose_success_data(share_freeze_list, 'share_freeze', 'share_freeze:current',
                                             key_no,
                                             from_queue, webpage_id)
                        ...
                        # 被执行人
                        zhixinglist_num = inquire_dict.get('被执行人')
                        if not zhixinglist_num:
                            zhixinglist_num = 0
                        if 0 < zhixinglist_num <= 10:
                            zhixinglist = susong_json["datalist"]["zhixinglist"]["data"]
                        # 多于10个的情况下
                        elif zhixinglist_num > 10:
                            zhixinglist = []
                            input("执行人超过十个，建议处理")
                            zhixinglist_page = math.ceil(zhixinglist_num / 10)
                            for page_ in range(1, zhixinglist_page + 1):
                                zhixinglist_url = f"https://www.qcc.com/api/datalist/zhixinglist?isNewAgg=true&keyNo={key_no}&pageIndex={page_}"
                                pass
                        else:
                            zhixinglist = []
                        dispose_success_data(zhixinglist, 'judgement_debtor', 'judgement_debtor:current', key_no,
                                             from_queue, webpage_id)

                        # 限制高消费
                        sumptuary_num = inquire_dict.get('限制高消费')
                        if not sumptuary_num:
                            sumptuary_num = 0
                        if 0 < sumptuary_num <= 10:
                            sumptuary_list = susong_json["datalist"]["sumptuarylist"]["data"]
                        # 多于10个的情况下
                        ...
                        # 股权质押
                        share_pledge_num = inquire_dict.get('股权质押')
                        if not share_pledge_num:
                            share_pledge_num = 0
                        if 0 < share_pledge_num <= 10:
                            share_pledge = cfengxian_json["datalist"]["spledge"]["data"]
                        # 多于10个的情况下
                        elif share_pledge_num > 10:
                            share_pledge = []
                            input("股权质押超过十个，建议处理")
                            share_pledge_page = math.ceil(share_pledge_num / 10)
                            for page_ in range(1, share_pledge_page + 1):
                                pass  # 暂未找到超过十个的
                        else:
                            share_pledge = []
                        dispose_success_data(share_pledge, 'share_pledge', 'share_pledge:current', key_no,
                                             from_queue, webpage_id)
                        ...
                        try:
                            ...
                            crun_ciaxlist = []
                        dispose_success_data(crun_ciaxlist, 'trade_credit', 'trade_credit:current', key_no, from_queue,
                                             webpage_id)
                        # 土地信息
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, landlist_page + 1):
                                landlist_url = f'https://www.qcc.com/api/datalist/landmergelist?isNewAgg=true&keyNo={key_no}&pageIndex={page_}'
                                landlist_value = get_response(landlist_url, key_no, pid, tid, cookie_dict)
                                captcha_value = encounter_captcha(landlist_value, page)
                                if captcha_value in [ '没有遇到验证码']:
                        ...
                        # 多于10个的情况下
                            ...
                            cassets_tmcdslist = []
                        dispose_success_data(cassets_tmcdslist, 'tm_doc', 'tm_doc:current', key_no, from_queue, webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            cassets_applist = []
                        dispose_success_data(cassets_applist, 'app', 'app:current', key_no, from_queue, webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            cassets_weibolist = []
                        dispose_success_data(cassets_weibolist, 'weibo', 'weibo:current', key_no, from_queue, webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hismainmember_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    # json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hismainmember_url, 'mihis_memberni_program', 'get', '', key_no,
                                                    webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hiszhixinglist_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    # json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hiszhixinglist_url, 'his_judgement_debtor', 'get', '', key_no,
                                                    webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hisadminpenaltylist_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    # json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hisadminpenaltylist_url, 'his_admin_punish', 'get', '', key_no,
                                                    webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hispledgelist_page + 1):
                                ...
                                hispledgelist_value = get_response(hispledgelist_url, key_no, pid, tid, cookie_dict)
                                captcha_value = encounter_captcha(hispledgelist_value, page)
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    # json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hispledgelist_url, 'his_equity_pledge', 'get', '', key_no,
                                                    webpage_id)
                        else:
                            chistory_hispledgelist = []
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hisshangbiaolist_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hisshangbiaolist_url, 'his_tm_info', 'post', json_data, key_no,
                                                    webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hiszhuanlilist_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hiszhuanlilist_url, 'his_patent_info', 'post', json_data, key_no,
                                                    webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            chistory_hiswebsitelist = []
                        # dispose_success_data(chistory_hiswebsitelist, 'his_patent_info', 'patent_info:history', key_no, from_queue, webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hisowenoticelist_page + 1):
                                hisowenoticelist_url = f'https://www.qcc.com/api/datalist/owenoticelist?isCombin=1&isValid=0&keyNo={key_no}&pageIndex={page_}&searchKey={key_no}'
                                pass
                        else:
                            chistory_hisowenoticelist = []
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hisendexecutioncaselist_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    # json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hisendexecutioncaselist_url, 'his_judgement_end', 'get', '', key_no,
                                                    webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hisshixinlist_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    # json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hisshixinlist_url, 'his_dishonest_debtor', 'get', '', key_no,
                                                    webpage_id)
                        ...
                        # 多于10个的情况下
                            ...
                            for page_ in range(1, hismpledgelist_page + 1):
                                ...
                                if captcha_value in [ '没有遇到验证码']:
                                    ...
                                    # 先将请求数据上传到数据库中，后续处理
                                    json_data = json.dumps(json_data, ensure_ascii=False)
                                    up_qcc_res_data(hismpledgelist_url, 'his_chattel_mortgage', 'post', json_data, key_no,
                                                    webpage_id)
                        ...
                        #     'his_legal_rep': chistory_operlist,  # 历史法定代表人
            ...
            # # 如果没有命中公司，将搜索页面的信息转换为 JSON 格式返回
...
def qcc_search_keyno(search_company_keyno, from_queue, webpage_id):
    ...
    if not pid_list or not tid_list:
        ...
        # return
        # 刚开始获取json数据
        if company_data and chistory_data:
            company_json = json.loads(company_data[0])
            # susong_json = json.loads(susong_data[0])
            # cfengxian_json = json.loads(cfengxian_data[0])
            # crun_json = json.loads(crun_data[0])
            ...
            """
            # 工商信息
            company_detail = company_json["company"]["companyDetail"]
            up_part_qcc_data(company_detail, 'business_info', key_no, from_queue, webpage_id)
            part_data = {
                'data_type': 'business_info:current',
                'id': from_queue,
            ...
            # 历史对外投资
            histouzilist_num = inquire_dict.get('历史对外投资')
            if not histouzilist_num:
                histouzilist_num = 0
            if histouzilist_num <= 50:
                # 少于10个的情况下
                if 0 < histouzilist_num <= 10:
                    chistory_histouzilist = chistory_json["datalist"]["histouzilist"]["data"]
                # 多于10个的情况下
