...
def qcc_search_company_chiyue(search_company_name):
    search_company_name = search_company_name
    hit_field_list = ['曾用名']  # 股东，曾用名
    random_num = random.randint(1, 6)

    co = ChromiumOptions()
    co = co.set_user_data_path(r"C:\chome_data\data_niyunqiu")
    # co = co.set_argument('--no-sandbox')
    # co = co.headless()
    co.set_paths(local_port=9163)
...
def qcc_search_keyno_chiyue(key_no):
    random_num = random.randint(1, 6)
