...
def get_image(page, url, element, is_to_bottom=False, left_offset=0, right_offset=0, up_offset=0, down_offset=0, is_click='', is_time=0, is_del_1='', is_del_2=''):
    tab = page.new_tab()
    tab.get(url)
    if is_time:
        time.sleep(is_time)
    ...
    time.sleep(2)
    if is_del_1:
        tab.remove_ele(is_del_1)
    if is_del_2:
        tab.remove_ele(is_del_2)
