...
def get_bensigonggao_data(queue_id, webpage_id):
    response = requests.get('https://www.samr.gov.cn/zljds/zlgg/bsgg/index.html', headers=headers)
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                for data in data_list:
                    ...
                    title_url = ''.join(data.xpath("./a/@href"))
                    if "http" not in title_url:
                    ...
                    if title_url not in url_set:
                        ...
                        if title_res.status_code == 200:
                            ...
                            if not annexs:
                                annexs = title_html.xpath("//div[@class='zt_xilan_02']//a/@href")
                            annexs_num = len(annexs)
                            if annexs_num > 0:
                                for annex in annexs:
                                    if len(annex) > 20 and "htm" not in annex:
                                        if "https" not in annex:
                                            annex = 'https://www.samr.gov.cn' + annex
                                        annex = re.sub(r'\?.*', '', annex)
                                        annex_url_type = annex.split('.')[-1]
                                        annex_value = upload_file_by_url(annex, "bhgcp", annex_url_type,
                                                                         type="other")
                                        annex_list += annex + ','
                                        if annex_value:
...
# get_bensigonggao_data(111, 222)

# paper_queue = paper_queue_next(webpage_url_list=["https://www.samr.gov.cn/zljds/zlgg/bsgg/index.html"])
...
#     try:
#         get_bensigonggao_data(queue_id, webpage_id)
#         data = {
#             "id": queue_id,
