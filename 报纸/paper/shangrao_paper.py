...
headers = {
    ...
    # 'Cookie': 'zycna=AyFFOCwXmvgBAXPBuY3xThSx',
    'Pragma': 'no-cache',
...
def get_shangrao_paper_new(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://paper.srxww.com/pc/column/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://paper.srxww.com/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
...
def get_shangrao_paper_old(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'http://paper.srxww.com/srrb/html/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://paper.srxww.com/srrb/' + "".join(
        ...
        raise Exception(f'该日期没有报纸')
# get_shangrao_paper_old('2024-01-01', 111, 111)





def get_shangrao_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    ...
    if paper_time1 <= date_str:
        # print('使用旧方法')
        get_shangrao_paper_old(paper_time, queue_id, webpage_id, bm_url_in)
    else:
        # print('使用新方法')
        get_shangrao_paper_new(paper_time, queue_id, webpage_id, bm_url_in)
