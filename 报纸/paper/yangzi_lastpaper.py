...
def get_yangzi_lastpaper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'https://epaper.yzwb.net/pc/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        # 获取所有版面的的链接
        all_bm = html_1.xpath("//div[@class='Chunkiconlist']/p/a[1]")
        for bm in all_bm:
            # 版面名称
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'https://epaper.yzwb.net/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
