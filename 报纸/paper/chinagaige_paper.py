...
def get_chinagaige_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://www.cfgw.net.cn/epaper/{paper_time}/'
    url = base_url + 'node_01.htm'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://www.cfgw.net.cn/epaper/' + "".join(bm_html.xpath("//div[@class='pull-right']/a/@href")).strip('../..')
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://www.cfgw.net.cn/epaper/' + ''.join(article.xpath("./@href")).strip('../..')
                # 获取文章名称
...
# get_chinagaige_paper('2023-08-21', 111, 1111)
