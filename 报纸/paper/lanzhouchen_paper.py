...
def get_lanzhouchen_paper_new(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://lzcbszb.benliuxinwen.com/{paper_time}/'
    url = base_url + 'colA01.html'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://lzcbszb.benliuxinwen.com/' + "".join(
                bm_html.xpath("//div[@class='nav-list']/ul/li/a[@class='pdf']/@href")).strip('../..')
...
def get_lanzhouchen_paper_old(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'http://morningpost.benliuxinwen.com/html/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 获取所有文章的链接
            all_article = bm_html.xpath("//div/table/tbody/tr/td[@class='default'][2]/a")
            pdf_set = set()
            for article in all_article:
                # 获取文章链接
...
# get_lanzhouchen_paper_old('2021-10-21', 111, 1111)
# get_lanzhouchen_paper_new('2024-08-22', 111, 1111)
def get_lanzhouchen_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    ...
    if paper_time1 <= date_str:
        # print('使用旧方法')
        get_lanzhouchen_paper_old(paper_time, queue_id, webpage_id, bm_url_in)
    else:
        # print('使用新方法')
        get_lanzhouchen_paper_new(paper_time, queue_id, webpage_id, bm_url_in)


# get_lanzhouchen_paper('2023-03-26', 111, 1111)
