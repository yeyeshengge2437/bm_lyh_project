...
def get_shenzhen_lastpaper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'https://wb.sznews.com/PC/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'https://wb.sznews.com/' + "".join(
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'https://wb.sznews.com/PC/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
...
# get_shenzhen_lastpaper('2018-11-20', 111, 1111)
