...
def get_linanminzu_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://szb.chinalxnet.com/pc/layout/{paper_time}/'
    url = base_url + 'node_01B.html'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://szb.chinalxnet.com/pc/' + "".join(bm_html.xpath('//*[@id="pdfUrl"]/text()')).strip('../../..')
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://szb.chinalxnet.com/pc/' + ''.join(article.xpath("./@href")).strip('../..')
                # 获取文章名称
...
# get_linanminzu_paper('2024-02-22', 111, 1111)
