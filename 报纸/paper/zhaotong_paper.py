...
def get_zhaotong_paper_new(paper_time, queue_id, webpage_id):
    ...
    base_url = f'https://dubao.ztnews.net/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'https://dubao.ztnews.net/' + ''.join(article.xpath("./@href")).strip('../..')
                # 获取文章名称
...
def get_zhaotong_paper_old(paper_time, queue_id, webpage_id):
    ...
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'https://old_dubao.ztnews.net/html/{paper_time}/'
    url = base_url + 'node_2.htm'
    response = requests.get(url, headers=headers_old, verify=False)
    if response.status_code == 200:
        content = response.content.decode()
        html_1 = etree.HTML(content)
        if not html_1:
            raise Exception(f'该日期没有报纸')
        # 获取所有版面的的链接
        all_bm = html_1.xpath("//td[2]//td[1]/table[1]//div[@class='scrollbar']//td[@class='default']")
        for bm in all_bm:
            # 版面名称
            ...
            # 版面的pdf
            bm_pdf = 'https://old_dubao.ztnews.net/' + "".join(
...
def get_zhaotong_paper(paper_time, queue_id, webpage_id):
    ...
    # 将字符串转换为日期对象
    date_str1 = datetime.strptime(date_str1, '%Y-%m-%d').date()
    date_str2 = datetime.strptime(date_str2, '%Y-%m-%d').date()
