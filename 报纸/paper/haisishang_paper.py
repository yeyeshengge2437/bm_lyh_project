...
def get_haisishang_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'https://hssb.fjdaily.com/pc/col/{paper_time}/'
    url = base_url + 'node_N01.html'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'https://hssb.fjdaily.com/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
