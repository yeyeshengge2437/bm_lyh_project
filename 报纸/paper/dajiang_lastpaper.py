...
def get_dajiang_lastpaper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'https://epaper.wuhunews.cn/pc/djwb/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'https://epaper.wuhunews.cn/pc/djwb/' + "".join(bm_html.xpath("//div[@class='pull-right']/a/@href")).strip('../../..')
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'https://epaper.wuhunews.cn/pc/djwb/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
...
# get_dajiang_lastpaper('2024-08-22', 111, 1111)
