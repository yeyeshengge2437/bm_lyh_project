...
def get_changji_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'http://pcepaper.cjxww.cn/html/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://pcepaper.cjxww.cn/' + "".join(
            ...
            # 获取所有文章的链接
            all_article = bm_html.xpath("//ul/li[@class='resultList']")
            pdf_set = set()
            for article in all_article:
                # 获取文章链接
                article_url = base_url + ''.join(article.xpath("./a[@class='pageLinks']/@href"))
                # 获取文章名称
                article_name = ''.join(article.xpath("./a[@class='pageLinks']/h4/text()")).strip()
...
# get_changji_paper('2024-08-22', 111, 1111)
