...
def get_langfangdushi_paper_new(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://epaper.lfcmw.com/dsbpaper/pc/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://epaper.lfcmw.com/dsbpaper/pc/' + "".join(
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://epaper.lfcmw.com/dsbpaper/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
...
def get_langfangdushi_paper_old(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'http://epaper.lfcmw.com/oldpaper/lfdsb/html/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://epaper.lfcmw.com/oldpaper/lfdsb/' + "".join(
...
def get_langfangdushi_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    ...
    if paper_time1 <= date_str:
        # print('使用旧方法')
        get_langfangdushi_paper_old(paper_time, queue_id, webpage_id, bm_url_in)
    else:
        # print('使用新方法')
        get_langfangdushi_paper_new(paper_time, queue_id, webpage_id, bm_url_in)

# get_langfangdushi_paper('2022-07-06', 111, 1111)