...
def get_yiyang_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m%d')
    base_url = f'https://epaper.yyrb.cn/{paper_time}/'
    url = base_url
    ...
    if response.status_code == 200:
        ...
        # 获取所有版面的的链接
        all_bm = html_1.xpath("//ul[@class='clearfix']/li/h3/a")
        for bm in all_bm:
            # 版面名称
            ...
            # 版面的pdf
            bm_pdf = base_url + "".join(bm_html.xpath("//a[@class='tool_btn_li yb']/@href"))

            # 获取所有文章的链接
            all_article = bm_html.xpath("//div[@class='title_list']/ul/li/h3/a")
            pdf_set = set()
            for article in all_article:
                ...
                article_id = re.findall(r'\d{6}', article_url)[-1]
                data = {
                    'id': f'{article_id}',
                    'classid': '93',
                }
...
# get_yiyang_paper('2024-12-01', 111, 1111)
