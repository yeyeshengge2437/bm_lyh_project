...
def get_zhangjiajie_lastpaper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://paper.zjjnews.cn:8081/zjjrbpc/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://paper.zjjnews.cn:8081/zjjrbpc/' + "".join(bm_html.xpath("//div[@class='nav-list']/ul/li/a[@class='pdf']/@href")).strip('../../..')
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://paper.zjjnews.cn:8081/zjjrbpc/' + ''.join(article.xpath("./@href")).strip('../..')
                # 获取文章名称
...
# get_zhangjiajie_lastpaper('2022-04-17', 111, 1111)
