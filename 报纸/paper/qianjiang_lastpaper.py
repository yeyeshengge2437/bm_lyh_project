...
headers = {
    ...
    # 'Cookie': 'Hm_lvt_419cfa1cc17e2e1dc6d4f431f8d19872=1727245154; HMACCOUNT=FDD970C8B3C27398; Hm_lpvt_419cfa1cc17e2e1dc6d4f431f8d19872=1727245163',
    'Pragma': 'no-cache',
...
def get_qianjiang_lastpaper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'http://qjwb.thehour.cn/html/{paper_time}/'
    url = base_url + 'node_77.htm'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            # 版面名称
            bm_name = "".join(bm.xpath("./dt/a[@id='pageLink']/text()")).strip()
            # 版面链接
            bm_url = base_url + ''.join(bm.xpath("./dt/a[@id='pageLink']/@href")).strip('./')
