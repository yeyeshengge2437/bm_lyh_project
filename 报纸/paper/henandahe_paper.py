...
pdf_domain = 'https://newpaper.dahe.cn/dhb/'
today = datetime.now().strftime('%Y-%m-%d')


def get_dahe_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'https://newpaper.dahe.cn/dhb/html/{paper_time}/'
...
# get_dahe_paper("2024-09-04", 111, 1111)
