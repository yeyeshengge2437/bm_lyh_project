...
co = ChromiumOptions()
co = co.set_paths(local_port=9261)
...
def get_fazhirebao_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m%d')
    base_url = f'http://epaper.legaldaily.com.cn/fzrb/content/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        # 获取所有版面的的链接
        all_bm = html_1.xpath("//td[2]/table[3]/tbody/tr")
        for bm in all_bm:
            # 版面名称
            bm_name = "".join(bm.xpath("./td[2]/a[@class='atitle']/text()")).strip()
            # 版面链接
            bm_url = base_url + ''.join(bm.xpath("./td[2]/a[@class='atitle']/@href"))
            # 版面的pdf
            bm_pdf = 'http://epaper.legaldaily.com.cn/fzrb/' + "".join(
                bm.xpath("./td[3]/a/@href")).strip('../..')

            # 获取版面详情
            cookies = get_paper_url_cookies(bm_url)
            bm_response = requests.get(bm_url, headers=headers, cookies=cookies, verify=False)
            time.sleep(1)
            bm_content = bm_response.content.decode()
            bm_html = etree.HTML(bm_content)

            # 获取所有文章的链接
            all_article = bm_html.xpath("//table[5]/tbody/tr/td[2]/a[@class='atitle']")
            pdf_set = set()
            for article in all_article:
                ...
                article_response = requests.get(article_url, headers=headers, cookies=cookies, verify=False)
                time.sleep(1)
                article_content = article_response.content.decode()
                article_html = etree.HTML(article_content)
                # 获取文章内容
                content = ''.join(article_html.xpath("//span[@id='oldcontenttext']/text()")).strip()
                # 上传到测试数据库
...
# get_fazhirebao_paper('2023-05-25', 111, 1111)
