...
def get_fuzhou_lastpaper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    year = paper_time[0:4]
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m%d')
    base_url = f'https://mag.fznews.com.cn/fzwb/{year}/{paper_time}/{paper_time}_plist.htm'
    url = base_url
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面链接
            bm_url = f'https://mag.fznews.com.cn/fzwb/{year}/{paper_time}/' + ''.join(bm.xpath("./td[1]/div[@class='bancititle']/a/@href")).strip('./')
            # 版面的pdf
            bm_pdf = f'https://mag.fznews.com.cn/fzwb/{year}/{paper_time}/' + "".join(
            ...
            for article in all_article:
                # 获取文章链接
                article_url = f'https://mag.fznews.com.cn/fzwb/{year}/{paper_time}' + ''.join(article.xpath(".//@href")).strip('..')
                # 获取文章名称
...
# get_fuzhou_lastpaper('2024-04-28', 111, 1111)
