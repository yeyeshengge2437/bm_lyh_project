...
headers = {
    ...
    'Pragma': 'no-cache',
    'Referer': 'http://cdrb.cdyee.com/pc/column/202410/17/node_A01.html',
...
def get_changde_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://rb.cdyee.com/pc/column/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://rb.cdyee.com/pc/' + "".join(
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://rb.cdyee.com/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
...
# get_changde_paper('2021-06-01', 111, 1111)
