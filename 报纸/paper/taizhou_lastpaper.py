...
def get_taizhou_lastpaper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://sz.tznews.cn/tzwb/pc/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            if bm_html is None:
                continue
            # 版面的pdf
            bm_pdf = 'http://sz.tznews.cn/tzwb/pc/' + "".join(bm_html.xpath("//img[@class='preview']/@src")).strip('../../..').strip('.1')
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://sz.tznews.cn/tzwb/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
...
# get_taizhou_lastpaper('2024-08-08', 111, 1111)
