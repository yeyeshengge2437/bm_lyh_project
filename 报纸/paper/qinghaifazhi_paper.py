...
def get_qinghaifazhi_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'https://www.qhfzb.com/epaper/pc/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'https://www.qhfzb.com/epaper/pc/' + "".join(bm.xpath("./a[2]/@href")).strip('../../..')
            # 获取版面详情
            ...
            # 获取所有文章的链接
            all_article = bm_html.xpath("//div[contains(@class, 'summary')]/h3/a")
            pdf_set = set()
            for article in all_article:
                # 获取文章链接
                article_url = 'https://www.qhfzb.com/epaper/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
                flag = 0
