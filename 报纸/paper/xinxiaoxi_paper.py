...
def get_xinxiaoxi_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'https://szb.nxrb.cn/xxxb/pc/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            if bm_html is None:
                continue
            # 版面的pdf
            bm_pdf = 'https://szb.nxrb.cn/xxxb/pc/' + "".join(bm_html.xpath("//div[@class='nav-list']/ul/li/a[@class='pdf']/@href")).strip('../../..')
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'https://szb.nxrb.cn/xxxb/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
...
# get_xinxiaoxi_paper('2023-02-09', 111, 1111)
