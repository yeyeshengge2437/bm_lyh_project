...
headers = {
    ...
    # 'Cookie': '__tins__4720970=%7B%22sid%22%3A%201727227384446%2C%20%22vd%22%3A%201%2C%20%22expires%22%3A%201727229184446%7D; __51cke__=; __51laig__=1',
    'Pragma': 'no-cache',
    'Referer': 'http://www.xnwbw.com/',
...
def get_xining_lastpaper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'http://www.xnwbw.com/html/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            # 版面名称
            bm_name = "".join(bm.xpath("./td[@class='font7']/a/text()")).strip()
            # 版面链接
            bm_url = base_url + ''.join(bm.xpath("./td[@class='font7']/a/@href"))
            # 版面的pdf
            bm_pdf = 'http://www.xnwbw.com/' + "".join(
