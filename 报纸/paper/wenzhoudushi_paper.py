...
def get_wenzhoudushi_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m-%d')
    data = {
        'digitalNewspaperMark': 'wzdsb',
        ...
        for bm in bm_list:
            ...
            if bm_response.status_code == 200:
                ...
                for article in article_list:
                    ...
                    article_url = f"http://szb.66wz.com/newspaper?mediaKey=wzdsb&showArticleDetail=true&docId={article_id}&curEdition={article['edition']}&publishTime={paper_time}"
