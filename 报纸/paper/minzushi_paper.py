...
def get_minzushi_paper_new(paper_time, queue_id, webpage_id):
    ...
    base_url = f'https://mzsb.yunnan.cn/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'https://mzsb.yunnan.cn/' + "".join(
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'https://mzsb.yunnan.cn/' + ''.join(article.xpath("./@href")).strip('../..')
                # 获取文章名称
...
def get_minzushi_paper_old(paper_time, queue_id, webpage_id):
    ...
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'https://mzsb.yunnan.cn/html/{paper_time}/'
    url = base_url + 'node_343.htm'
    response = requests.get(url, headers=headers)
    if response.status_code == 200:
        content = response.content.decode()
        html_1 = etree.HTML(content)
        # 获取所有版面的的链接
        all_bm = html_1.xpath("//div[@id='div']/table/tbody/tr")
        for bm in all_bm:
            # 版面名称
...
# get_minzushi_paper('2021-08-20', 111, 1111)
