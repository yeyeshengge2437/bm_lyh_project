...
def get_minnan_paper_new(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://zzxww.com/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        # 获取所有版面的的链接
        all_bm = html_1.xpath("//li[@class='posRelative']")
        for bm in all_bm:
            # 版面名称
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://zzxww.com/' + ''.join(article.xpath("./@href")).strip('../..')
                # 获取文章名称
...
def get_minnan_paper_old(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y-%m/%d')
    base_url = f'http://szb.zzxww.com/html/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        html_1 = etree.HTML(content)
        # 获取所有版面的的链接
        all_bm = html_1.xpath("//tr/td[@class='default']/a[@id='pageLink']")
        for bm in all_bm:
            # 版面名称
            ...
            # 版面的pdf
            bm_pdf = 'http://szb.zzxww.com/' + "".join(
            ...
            # 获取所有文章的链接
            all_article = bm_html.xpath("//div/table/tbody/tr/td[@class='default'][2]")
            pdf_set = set()
            for article in all_article:
                # 获取文章链接
...
def get_minnan_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    ...
    if paper_time1 <= date_str:
        # print('使用旧方法')
        get_minnan_paper_old(paper_time, queue_id, webpage_id, bm_url_in)
    else:
        # print('使用新方法')
        get_minnan_paper_new(paper_time, queue_id, webpage_id, bm_url_in)

# get_minnan_paper('2020-08-09', 111, 222)
