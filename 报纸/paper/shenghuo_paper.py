...
headers = {
    ...
    'Pragma': 'no-cache',
    'Referer': 'http://epaper.hljnews.cn/shb/pc/layout/',
...
def get_shenghuo_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'http://epaper.hljnews.cn/shb/pc/layout/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_pdf = 'http://epaper.hljnews.cn/shb/pc/' + "".join(
            ...
            for article in all_article:
                # 获取文章链接
                article_url = 'http://epaper.hljnews.cn/shb/pc/' + ''.join(article.xpath("./@href")).strip('../../..')
                # 获取文章名称
...
# get_shenghuo_paper('2021-07-21', 111, 1111)
