...
def get_xihaidushi_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m/%d')
    base_url = f'https://epaper.tibet3.com/xhdsb/html/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        # 获取所有版面的的链接
        all_bm = html_1.xpath("//li/a[@class='nav-item-links']")
        for bm in all_bm:
            # 版面名称
            ...
            # 获取所有文章的链接
            all_article = bm_html.xpath("//ul/li/a[@class='news-item']")
            pdf_set = set()
            for article in all_article:
                ...
                # 获取文章内容
                content = ''.join(article_html.xpath("//div[@class='article-entry']/p/text()")).strip()
                # 上传到测试数据库
