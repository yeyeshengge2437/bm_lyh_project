...
def get_huashang_paper(paper_time, queue_id, webpage_id, bm_url_in=None):
    # 将today的格式进行改变
    day = paper_time
    paper_time = datetime.strptime(paper_time, '%Y-%m-%d').strftime('%Y%m%d')
    base_url = f'http://ehsb.hspress.net/shtml/hsb/{paper_time}/'
    ...
    if response.status_code == 200:
        ...
        for bm in all_bm:
            ...
            # 版面的pdf
            bm_img = "".join(bm_html.xpath("//div[@id='paperPage']/img/@src")).strip()
            bm_img = re.sub(r'b_', '', bm_img)
            bm_img = "http://ehsb.hspress.net" + bm_img
            ...
            for article in all_article:
                ...
                # 获取文章内容
                content = ''.join(article_html.xpath("//div[@id='art']//text()")).strip()
