...
for notice in all_notice[:-1]:
    ...
    print(f"案件号：{case_no}，案由：{cause}，法院：{court}，开庭时间：{open_time}，庭室：{court_room}，审判人员：{members}，创建时间：{create_time}，创建日期：{create_date}，来源：{origin}，来源域名：{origin_domain}")
    data_unique = case_no
    # 数据去重
    ...
    if not redis_conn.sismember("guangzhou_set", hash_value):
        ...
        insert_sql = "INSERT INTO case_open_copy1 (case_no, cause, court, members, open_time, court_room,  origin, origin_domain, create_time, create_date) VALUES (%s,  %s, %s, %s, %s, %s, %s, %s, %s, %s)"
